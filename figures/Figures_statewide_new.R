...
dstk = dstk_1to2
MHO = MHO_1to2
plot_timetide = function(MHO, dstk)
{
	# only use data after 1995
	dstk = dstk[dstk$yr > 1995,]
	# make sure polyid and stockid are factors in count data
	...
	beta0 = unlist(lapply(MHO[['beta']], function(x){x[[1]]}))
	beta1 = unlist(lapply(MHO[['beta']], function(x){x[[6]]}))
	beta2 = unlist(lapply(MHO[['beta']], function(x){x[[7]]}))
#  par(mar = c(6,7,8,3))
	par(mar = c(0,7,6,3))
  ...
  bot = apply(HOvec,2,quantile, probs = .05)
  top = apply(HOvec,2,quantile, probs = .95)
	polygon(x = c(xtran,rev(xtran)),
		y = c(top,rev(bot)), col = 'grey', border = 'grey')
	lines(xtran, apply(HOvec,2,mean), lwd = 4)
	mtext('Haul-out Probability', side = 2, cex = cex_lab, 
		adj = adj_2, padj = padj_2)
	par(mar = c(6,7,0,3))
	hist(as.numeric(dstk$minutes_from_low)/60, main = '',
		xaxt = 'n', cex.lab = cex_lab, cex.axis = cex_axis, xlab = '',
		ylab = '', yaxt = 'n', xlim = c(-5,5),
		breaks = (-5:5)) 
	box()
#	mtext('Freq', side = 2, cex = cex_lab, 
#		adj = adj_2, padj = padj_2)
...
layout(matrix(1:24, ncol = 3), heights = c(2,1,2,1,2,1,2,1))
...
plot_timeday(MHO_1to2, dstk_1to2)
plot_timeday(MHO_3to7, dstk_3to7)
plot_timeday(MHO_8to12, dstk_8to12)
plot_timeday(MHO_glac, dGlac)
plot_timetide(MHO_1to2, dstk_1to2)
plot_timetide(MHO_3to7, dstk_3to7)
plot_timetide(MHO_8to12, dstk_8to12)
par(xpd=NA)
text(x = -15, y = 31900, labels = substitute(paste(bold('Stocks 1-2: Aleutian Islands and Pribilof Islands'))), cex = 2.1)
text(x = -15, y = 20100, labels = substitute(paste(bold('Stocks 3-7: Bristol Bay, N Kodiak, S Kodiak, Prince William Sound, Cook Inlet/Shelikof Strait'))), cex = 2.1)
text(x = -15, y = 8400, labels = substitute(paste(bold('Stocks 8-12: Glacier Bay/Icy Str, Lynn Canal/Stephens Psg, Sitka/Chatham Str, Dixon/Cape Decision, Clarence Str'))), cex = 2.1)
text(x = -15, y = -3200, labels = substitute(paste(bold('Glacial Fjords in Stocks 6, 8, 9, 12'))), cex = 2.1)
